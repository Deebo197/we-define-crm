  ...
  return (
    <div>
      ...
      {confirmDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-surface rounded-2xl shadow-card border border-line p-6 max-w-sm w-full mx-4 shadow-2xl">
            <h3 className="text-ink font-medium mb-2">Delete Report</h3>
            <p className="text-muted text-sm mb-5">Are you sure you want to delete <span className="text-ink font-medium">{confirmDelete.title}</span> and both its versions? This cannot be undone.</p>
      ...
      <div className="relative mb-6 animate-fade-in-up" style={{ animationDelay: "0.05s" }}>
        ...
        <Input placeholder="Search reports..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-10 bg-surface border-line text-ink placeholder:text-faint rounded-xl h-10" />
      </div>
      ...
      {isLoading ? <ShimmerCard count={3} /> : filtered.length === 0 ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((report, i) => (
            <div key={report.id} className="bg-surface rounded-2xl shadow-card border border-line p-5 hover:border-line-strong hover:scale-[1.01] transition-all duration-300 cursor-pointer animate-fade-in-up group relative" style={{ animationDelay: `${i * 0.03}s` }} onClick={() => setViewing(report)}>
              <button type="button" onClick={(e) => { e.stopPropagation(); setConfirmDelete(report); }} className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 p-1.5 rounded-lg text-faint hover:text-danger hover:bg-danger/10 transition-all">
                <Trash2 className="w-3.5 h-3.5" />
              </button>
              <div className="flex items-start justify-between mb-2 pr-6">
                <div>
                  <h3 className="text-ink font-medium text-sm group-hover:text-primary transition-colors">{report.title}</h3>
                  <p className="text-faint text-xs mt-0.5">{report.client_name} · {report.month}</p>
                </div>
              ...
              {report.metrics ? (
                ...
                <p className="text-faint text-xs mt-2 italic">No metrics yet — open to auto-draft</p>
              )}
            </div>
          ))}
        </div>
