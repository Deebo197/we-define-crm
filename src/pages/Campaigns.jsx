  ...
  return (
    <div>
      ...
      {confirmDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-surface rounded-2xl shadow-card border border-line p-6 max-w-sm w-full mx-4 shadow-2xl">
            <h3 className="text-ink font-medium mb-2">Delete Campaign</h3>
      ...
      <div className="relative mb-6 animate-fade-in-up" style={{ animationDelay: "0.05s" }}>
        ...
        <Input placeholder="Search campaigns..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-10 bg-surface border-line text-ink placeholder:text-faint rounded-xl h-10" />
      </div>
      ...
      {isLoading ? <ShimmerCard count={4} /> : filtered.length === 0 ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {filtered.map((campaign, i) => (
            <div key={campaign.id} className="bg-surface rounded-2xl shadow-card border border-line p-5 hover:border-line-strong hover:scale-[1.01] transition-all duration-300 cursor-pointer animate-fade-in-up group relative" style={{ animationDelay: `${i * 0.03}s` }} onClick={() => setViewing(campaign)}>
              <button type="button" onClick={(e) => { e.stopPropagation(); setConfirmDelete(campaign); }} className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 p-1.5 rounded-lg text-faint hover:text-danger hover:bg-danger/10 transition-all">
                <Trash2 className="w-3.5 h-3.5" />
              </button>
              <div className="flex items-start justify-between mb-2 pr-6">
                <div>
                  <h3 className="text-ink font-medium text-sm group-hover:text-primary transition-colors">{campaign.name}</h3>
              ...
              <div className="flex items-center flex-wrap gap-3 mt-3">
                {campaign.funding_type && (
                  <span className="text-muted text-xs">{campaign.funding_type}</span>
                )}
                {campaign.budget && (
                  <span className="text-muted text-xs">£{campaign.budget.toLocaleString()}</span>
                )}
                {campaign.start_date && (
              ...
              {(campaign.linked_client_names?.length > 0 || campaign.linked_partner_names?.length > 0) && (
                <div className="flex flex-wrap gap-1 mt-2">
                  ...
                  {campaign.linked_partner_names?.map(name => (
                    <span key={name} className="px-2 py-0.5 rounded-full text-[10px] bg-success/10 text-success">{name}</span>
                  ))}
                </div>
              )}
            </div>
