...
export default function GlobalSearchDialog() {
  ...
  const results = useMemo(() => {
    ...
    return [...companyHits, ...peopleHits].slice(0, 8);
  }, [query, companies, people]);
      ...
      <div className="bg-surface rounded-2xl border border-line shadow-2xl w-full max-w-lg overflow-hidden animate-fade-in-up">
        ...
        <div className="max-h-[50vh] overflow-y-auto">
          {query.trim() && results.length === 0 && (
            <p className="text-faint text-sm px-4 py-6 text-center">No companies or people match “{query}”</p>
          )}
          {!query.trim() && (
            <p className="text-faint text-xs px-4 py-5 text-center">Start typing to search across companies and people</p>
          )}
          {grouped.map((group) => (
