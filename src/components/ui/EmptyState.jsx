...
export default function EmptyState({ icon: Icon, title, description, action, actionLabel }) {
  return (
    <div className="flex flex-col items-center justify-center py-16 px-4 text-center animate-fade-in-up">
      ...
      <p className="text-faint text-sm max-w-sm mb-6">{description}</p>
      {action && (
        <Button
          onClick={action}
          className="bg-primary hover:bg-primary-hover text-white border-0 rounded-xl px-5 h-10 text-sm"
        >
