...
import { buildReportPrintHtml, REPORT_SECTIONS as SECTIONS } from "./reportHtml";
...
Write 2-4 short paragraphs of flowing prose in UK English. No bullet points, no headings, no preamble — start straight with the content. Mention sources inline where known.`,
  ...
  const handlePdfExport = () => {
    ...
    if (!w) {
      toast({ title: "Export blocked", description: "Allow pop-ups and try again.", variant: "destructive" });
      return;
    }
    w.opener = null;
    w.document.write(buildReportPrintHtml(report, internalVersion));
    w.document.close();
